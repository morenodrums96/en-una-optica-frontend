  ...
  return (
    <div className="grid grid-cols-2 gap-4 p-5 bg-white dark:bg-primary-900 rounded-xl border border-primary-200 dark:border-primary-700 shadow-md hover:shadow-lg transition-shadow duration-300">
        ...
        onChange={async (e) => {
          ...
          if (/^\d*\.?\d{0,2}$/.test(value) || value === '') {
            ...
            if (!isNaN(numericValue)) {
              try {
                const customerPrice = await getCustomerPrice(numericValue)
      ...
      <textarea
        ...
        className={`w-full px-4 py-2 border rounded-lg transition col-span-2 ${errors.description ? 'border-red-500 focus:ring-red-400' : 'border-gray-300 dark:border-primary-700 focus:ring-primary-400'
...
function inputClass(hasError?: string) {
  return `w-full px-4 py-2 border rounded-lg transition ${hasError ? 'border-red-500 focus:ring-red-400' : 'border-gray-300 dark:border-primary-700 focus:ring-primary-400'
    } placeholder-gray-400 dark:placeholder-gray-500 bg-white dark:bg-primary-800 text-gray-900 dark:text-white`
}
